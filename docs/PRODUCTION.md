...
- The new corners save and reload: movie-night choices/venues, adventure photos/pages, lyric-wall text/styles/order, little-loves stories and lines, and moon-days period dates/flow with the wheel, cycle day, expected window, and weigh-ins.
...
- Song downloads resolve full tracks only from open catalogs (Jamendo with a client id, Audius artist-marked downloads, Internet Archive audio) and stream them through the backend; songs outside those catalogs report a miss instead of falling back to clips or rips. On a miss, either member can attach their own audio file (up to 60 MB) to the song entry; a manual file outranks the catalogs, replaces cleanly, and serves from the room's media storage.
